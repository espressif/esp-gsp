...
gspc pack \
  examples/usage/widgets/layer/layer.json \
...
动态 `opacity` 更新图层自身绘制的透明度，不是整棵子树的整体透明度。仅承载子节点、没有自身视觉内容的图层不支持动态 `w`、`h`、`radius`、`opacity`，编译器会报错；这类分组可使用动态 `x`/`y` 移动子树。动态属性的声明与范围见[场景指南](../guide/scenes.md#动态属性范围)。
...
该文件来自 `examples/usage/widgets/layer/layer.json`。复制时请一并复制它引用的相对资源。
