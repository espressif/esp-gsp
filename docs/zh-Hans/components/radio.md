...
每个 radio 独立保存 `checked` 状态；点击一个不会清除其他选项。需要互斥单选时，为每个选项设置 `name`，并由应用通过生成的 `set_checked` 函数将选中项设为 `true`、其余项设为 `false`。
...
该文件来自 `examples/usage/widgets/radio/radio.json`。复制时请一并复制它引用的相对资源。
