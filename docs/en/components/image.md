...
Static or animated raster asset with contain, cover and stretch fitting.
...
```c
const gsp_component_directory_t *const * gsp_image_docs_component_directories(uint16_t *out_count)
esp_err_t gsp_widget_image_sample_get_effective_visible(esp_gsp_handle_t gsp, bool *out_visible)
esp_err_t gsp_widget_image_sample_get_info(esp_gsp_handle_t gsp, esp_gsp_component_info_t *out_info)
esp_gsp_config_t gsp_image_docs_config(void)
size_t gsp_image_docs_dynamic_image_slots(void)
```
...
| `w` | `int` | yes | 0…65535 | scene: SVG; template: — | width in px |
| `h` | `int` | yes | 0…65535 | scene: SVG; template: — | height in px |
