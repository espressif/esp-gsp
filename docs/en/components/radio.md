...
Circular boolean choice visual commonly coordinated by application state.
...
gspc pack \
  examples/usage/widgets/radio/radio.json \
  --deployable -o gsp-out/widget-preview/radio.gspb
gsp_sim_host \
  --bundle gsp-out/widget-preview/radio.gspb
```
...
This is `examples/usage/widgets/radio/radio.json`. Copy any relative assets referenced by the scene with it.

## Generated C API for this example

```c
const gsp_component_directory_t *const * gsp_radio_docs_component_directories(uint16_t *out_count)
esp_err_t gsp_widget_radio_option_a_get_checked(esp_gsp_handle_t gsp, bool *out_checked)
esp_err_t gsp_widget_radio_option_a_get_effective_visible(esp_gsp_handle_t gsp, bool *out_visible)
esp_err_t gsp_widget_radio_option_a_get_info(esp_gsp_handle_t gsp, esp_gsp_component_info_t *out_info)
esp_err_t gsp_widget_radio_option_a_get_value(esp_gsp_handle_t gsp, int32_t *out_value)
esp_err_t gsp_widget_radio_option_a_set_checked(esp_gsp_handle_t gsp, bool checked)
esp_err_t gsp_widget_radio_option_a_set_value(esp_gsp_handle_t gsp, int32_t value)
esp_gsp_config_t gsp_radio_docs_config(void)
size_t gsp_radio_docs_dynamic_image_slots(void)
```
...
| `fg_color` | `color` | — | default #55A0E8 | — | foreground color (text/knob/line/mark per type) |
