# Progress (`gsp_progress`)

Read-only horizontal or vertical representation of a ranged value.
...
gspc pack \
  examples/usage/widgets/progress/progress.json \
  --deployable -o gsp-out/widget-preview/progress.gspb
gsp_sim_host \
  --bundle gsp-out/widget-preview/progress.gspb
```
...
This is `examples/usage/widgets/progress/progress.json`. Copy any relative assets referenced by the scene with it.

## Generated C API for this example

```c
const gsp_component_directory_t *const * gsp_progress_docs_component_directories(uint16_t *out_count)
esp_err_t gsp_widget_progress_download_get_effective_visible(esp_gsp_handle_t gsp, bool *out_visible)
esp_err_t gsp_widget_progress_download_get_info(esp_gsp_handle_t gsp, esp_gsp_component_info_t *out_info)
esp_err_t gsp_widget_progress_download_get_value(esp_gsp_handle_t gsp, int32_t *out_value)
esp_err_t gsp_widget_progress_download_set_value(esp_gsp_handle_t gsp, int32_t value)
esp_gsp_config_t gsp_progress_docs_config(void)
size_t gsp_progress_docs_dynamic_image_slots(void)
```
