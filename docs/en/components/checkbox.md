# Checkbox (`gsp_checkbox`)

Independent checked state with a square visual affordance.
...
gspc pack \
  examples/usage/widgets/checkbox/checkbox.json \
  --deployable -o gsp-out/widget-preview/checkbox.gspb
gsp_sim_host \
  --bundle gsp-out/widget-preview/checkbox.gspb
```
...
This is `examples/usage/widgets/checkbox/checkbox.json`. Copy any relative assets referenced by the scene with it.

## Generated C API for this example

```c
const gsp_component_directory_t *const * gsp_checkbox_docs_component_directories(uint16_t *out_count)
esp_err_t gsp_widget_checkbox_notifications_get_checked(esp_gsp_handle_t gsp, bool *out_checked)
esp_err_t gsp_widget_checkbox_notifications_get_effective_visible(esp_gsp_handle_t gsp, bool *out_visible)
esp_err_t gsp_widget_checkbox_notifications_get_info(esp_gsp_handle_t gsp, esp_gsp_component_info_t *out_info)
esp_err_t gsp_widget_checkbox_notifications_get_value(esp_gsp_handle_t gsp, int32_t *out_value)
esp_err_t gsp_widget_checkbox_notifications_set_checked(esp_gsp_handle_t gsp, bool checked)
esp_err_t gsp_widget_checkbox_notifications_set_value(esp_gsp_handle_t gsp, int32_t value)
esp_gsp_config_t gsp_checkbox_docs_config(void)
size_t gsp_checkbox_docs_dynamic_image_slots(void)
```
...
| `fg_color` | `color` | — | default #50B878 | — | foreground color (text/knob/line/mark per type) |
