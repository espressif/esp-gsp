...
python -m gsp.execute --version 0.2.8 gspc pack \
  examples/widgets/arc/arc.json \
  --deployable -o build/widget-preview/arc.gspb
python -m gsp.execute --version 1.1.0 sim \
  --bundle build/widget-preview/arc.gspb
```
...
This is `examples/widgets/arc/arc.json`. Copy any relative assets referenced by the scene with it.
...
| `start_angle` | `int` | — | default 135; 0…359 | yes | arc start angle in degrees |
| `sweep` | `int` | — | default 270; 1…360 | — | arc sweep in degrees |
