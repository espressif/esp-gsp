...
#include "hw_init.h"
/* Status queries use scene-qualified target IDs from the bundle header. */
#define GSP_BUNDLE_ENABLE_RAW_IDS
#include "bundle_gsp.h"
#include "media.h"
static const char *TAG = "asset_example";

/* Bounded startup diagnostics on the application task, not the UI task.
 * Playback continues after this check; later failures are logged by GSP and
 * can be queried through the same status API in the application's lifecycle. */
static void check_startup_assets(esp_gsp_assets_t *assets, esp_gsp_handle_t ui)
{
    const uint16_t binds[] = { GSP_GALLERY_BIND_SPINNER, GSP_GALLERY_BIND_FADE };
    const char *names[] = { "spinner", "fade" };
    ...
    while (esp_timer_get_time() < deadline) {
        ...
        for (size_t i = 0; i < sizeof(binds) / sizeof(binds[0]); ++i) {
            ...
            esp_gsp_err_t ret = esp_gsp_assets_get_status(assets, ui,
                                esp_gsp_asset_image_target(binds[i]), &status);
            if (ret != ESP_GSP_OK) {
                ESP_LOGW(TAG, "%s: status unavailable (%d)", names[i], ret);
                checked[i] = true;
            } else if (status.state == ESP_GSP_ASSET_FAILED) {
                const char *stage = status.failed_stage == ESP_GSP_ASSET_READING ? "read/validate" : "decode/publish";
                ESP_LOGW(TAG, "%s: error=%d stage=%s errno=%d encoded_required=%zu; previous image retained",
                         names[i], status.last_error, stage,
                         status.system_errno, status.required_bytes);
                checked[i] = true;
            } else if (status.state == ESP_GSP_ASSET_STOPPED) {
                ESP_LOGW(TAG, "%s: stopped (error=%d)", names[i], status.last_error);
                checked[i] = true;
            } else if (status.state == ESP_GSP_ASSET_READY || status.state == ESP_GSP_ASSET_FINISHED) {
                ESP_LOGI(TAG, "%s: frame ready", names[i]);
                checked[i] = true;
            } else {
                pending = true;
            }
    ...
    for (size_t i = 0; i < sizeof(binds) / sizeof(binds[0]); ++i) {
        if (!checked[i]) {
            ESP_LOGW(TAG, "%s: no ready state observed within startup check; service remains active", names[i]);
        }
    }
    esp_gsp_assets_stats_t stats;
    if (esp_gsp_assets_get_stats(assets, ui, &stats) == ESP_GSP_OK) {
        ESP_LOGI(TAG, "Asset memory: encoded=%zu/%zu decoded=%zu index=%zu (excludes display, task and codec scratch)",
                 stats.encoded_bytes, stats.encoded_budget_bytes, stats.decoded_bytes, stats.index_bytes);
    }
}
...
void app_main(void)
{
    ...
    ret = gsp_gallery_spinner_set_asset(ui, assets, gsp_media_spinner, false);
    if (ret != ESP_GSP_OK) {
        ESP_LOGW(TAG, "spinner: request rejected (%d); built-in image remains", ret);
    }
    ret = gsp_gallery_fade_set_asset(ui, assets, gsp_media_fade, false);
    if (ret != ESP_GSP_OK) {
        ESP_LOGW(TAG, "fade: request rejected (%d); built-in image remains", ret);
    }
    check_startup_assets(assets, ui);
