...
已有过渡完成后保持选中状态。`set_<属性>_rgb888()` 统一接收 `0xRRGGBB`，
自动适配场景格式；特效前景色使用 `set_fg_color_rgb888()`。不带 `_rgb888` 的颜色 setter/getter 使用
原生颜色值。透明度单独设置，RGB888 接口不接受 alpha 字节。背景颜色仍是构建期配置。
...
这些控制能力使用 GSB 2.12，工具版本使用上述兼容性指南中的配套组合。
