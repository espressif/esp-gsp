...
def asset_content_equal(path, generated):
    """Compare generated images without depending on encoder metadata."""
    if path.suffix.lower() not in {".gif", ".png"}:
        return path.read_bytes() == generated
    with Image.open(path) as existing, Image.open(io.BytesIO(generated)) as candidate:
        if (existing.size != candidate.size or
                getattr(existing, "n_frames", 1) != getattr(candidate, "n_frames", 1)):
            return False
        existing_loop = existing.info.get("loop")
        candidate_loop = candidate.info.get("loop")
        if existing_loop != candidate_loop:
            return False
        for frame in range(getattr(existing, "n_frames", 1)):
            existing.seek(frame)
            candidate.seek(frame)
            if existing.info.get("duration") != candidate.info.get("duration"):
                return False
            if existing.convert("RGBA").tobytes() != candidate.convert("RGBA").tobytes():
                return False
    return True
...
def main():
    ...
    for name, content in generated_assets.items():
        path = Path(name)
        if args.check:
            if not path.exists() or not asset_content_equal(path, content):
