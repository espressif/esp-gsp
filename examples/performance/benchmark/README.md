...
1. In `examples/performance/benchmark`, select your board's [build profile](#build) below
...
4. For repeatable qualification, use [matrix capture](#capture-a-board-matrix)
   ...
   to repeat one case longer, or the [native C UI backend](#native-c-ui-backend)
...
`pc/` directly compiles `main/bench_workload.c` with
...
python managed_components/espressif__esp-gsp/tools/sim_bridge/run.py \
  --project managed_components/espressif__esp-gsp/examples/performance/benchmark/pc
```
...
The composites page gives each tab 15 seconds on C3 (45 seconds total) and
12 seconds on other targets (36 seconds total). `keyboard_window_ms` in the
configuration fingerprint records the per-tab window. It drives the keyboard
at a 4 ms event cadence with a 255-byte edit buffer. Protocol 16 and
later require the final verified text to exceed the 63-byte command inline
threshold; the parser rejects runs that do not reach that boundary. During long
...
Use the matching GSPC, ESP-GSP and simulator versions in the
[compatibility guide](../../../docs/en/reference/compatibility.md).
...
Missing industrial saturation artwork is restored from the pinned example
archive at build time; see [asset retrieval](../../README.md#example-asset-retrieval).
It has no runtime network dependency. The check validates exact dimensions
and alpha coverage.
