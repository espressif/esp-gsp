...
All runtime assets are local; building and running the benchmark does not
download images. The two industrial images were generated using the built-in
image-generation tool, not the API/CLI fallback. They replace the previous RGB
and alpha artwork, including references from the image/grid widget examples.
No external stock-photo download was used.
...
The generated outputs were normalized to the checked-in dimensions above.
